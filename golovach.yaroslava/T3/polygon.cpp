...
bool isSameShape(const Polygon& a, const Polygon& b)
{
  if (a.points.size() != b.points.size())
    return false;
  if (a.points.empty())
    return true; // не встречается (минимум 3 вершины)

  int dx = b.points[0].x - a.points[0].x;
  int dy = b.points[0].y - a.points[0].y;

  return std::equal(a.points.begin(), a.points.end(),
                    b.points.begin(),
                    [dx, dy](const Point& p1, const Point& p2) {
                      return (p1.x + dx == p2.x) && (p1.y + dy == p2.y);
                    });
}

SamePred::SamePred(const Polygon& t) : target(t) {}

bool SamePred::operator()(const Polygon& p) const
{
