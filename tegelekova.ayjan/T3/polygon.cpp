...
namespace tchervinsky
{
  ...
  bool isRectangle(const Polygon& poly)
  {
    ...
    // Проверка, что все углы прямые (через скалярное произведение)
    for (size_t i = 0; i < 4; ++i)
    {
      const Point& p1 = poly.points[i];
      const Point& p2 = poly.points[(i + 1) % 4];
      const Point& p3 = poly.points[(i + 2) % 4];
  ...
  bool hasRightAngle(const Polygon& poly)
  {
    ...
    for (size_t i = 0; i < n; ++i)
    {
      const Point& p1 = poly.points[i];
      const Point& p2 = poly.points[(i + 1) % n];
      const Point& p3 = poly.points[(i + 2) % n];
